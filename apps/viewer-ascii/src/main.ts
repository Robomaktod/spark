 ...
 * them through the same ReplayPlayer the web viewer uses, which means watching a
...
import { describeEvent, type Round } from '@spark/engine';
import { ReplayPlayer, isReplayFile, type ReplayFile } from '@spark/replay';
...
async function main(): Promise<void> {
  ...
  if (!args) {
    process.stderr.write(
      'usage: spark-view <replay.json> [--speed MS] [--round N] [--dump] [--no-colour]\n',
    );
    process.exitCode = 2;
    return;
  }
  ...
  const totalRounds = rules.match.games * rules.match.roundsPerGame;

  while (!player.finished) {
    const f = player.step(false);
    if (!f) break;
    // The event log lives in the replay, generated by the engine. Both viewers
    // read the same lines, so they cannot describe the same match differently.
    for (const e of f.events) eventLines.push(describeEvent(e));
